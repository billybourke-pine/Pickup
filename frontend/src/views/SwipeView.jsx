...
export default function SwipeView({ games, filters, onPass, onInterested, onConfirm, stats, isAdmin, onPin, onDelete, siteConfig, currentUid }) {
  const [index, setIndex] = useState(0);
  useEffect(()=>{ setIndex(0); },[filters]);

  const filtered = games.filter(g => {
    const isMine = currentUid && g.hostUid === currentUid;
    ...
    <section className={s.view}>
      <div className={s.topbar}>
        <div className={s.brand}>
          ...
          <div><strong>{siteConfig?.appName || 'Pickup'}</strong><span>{siteConfig?.tagline || 'Swipe into your next game.'}</span></div>
        </div>
      </div>

      <div className={s.hero}>
        <span className={s.eyebrow}>{siteConfig?.heroKicker || 'Live near you'}</span>
        <h1>{siteConfig?.heroTitle || 'Find a game fast.'}</h1>
        ...
        {deck.length===0
          ...
          : deck.map((g,i) => (
              <SwipeCard key={g.id} game={g} depth={i+1} isTop={i===0}
                isAdmin={isAdmin} onPin={onPin} onDelete={onDelete}
      ...
      <div className={s.actionRow}>
        <button id="passBtn" className={`${s.btn} ${s.pass}`} onClick={()=>top&&(onPass(top),next())} aria-label="Pass" data-testid="pass-btn">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
        </button>
        <button id="confirmBtn" className={`${s.btn} ${s.confirm}`} onClick={()=>top&&(onConfirm(top),next())} aria-label="Confirm spot" data-testid="confirm-btn">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 5v14M5 12h14"/></svg>
          <span>Confirm</span>
        </button>
        <button id="likeBtn" className={`${s.btn} ${s.like}`} onClick={()=>top&&(onInterested(top),next())} aria-label="Interested" data-testid="interested-btn">
